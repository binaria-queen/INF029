  ...
  if (posicao_invalida == POSICAO_INVALIDA)
    ...
    if (vetorPrincipal[posicao].vetorAuxiliar != NULL) {
      if (vetorPrincipal[posicao].qtd < vetorPrincipal[posicao].tamanho) {
        // insere
        vetorPrincipal[posicao].vetorAuxiliar[vetorPrincipal[posicao].qtd] =
            valor;
...
int excluirNumeroEspecificoDeEstrutura(int posicao, int valor) {
  ...
  for (int j = i; j < tamanho - 1; j++) {
    vetorPrincipal[posicao].vetorAuxiliar[j] =
        vetorPrincipal[posicao].vetorAuxiliar[j + 1];
  }
...
int modificarTamanhoEstruturaAuxiliar(int posicao, int novoTamanho) {
  ...
  int *novoVetor =
      realloc(vetorPrincipal[posicao].vetorAuxiliar, tamanhoNovo * sizeof(int));
  if (novoVetor == NULL)
    return SEM_ESPACO_DE_MEMORIA;
...
void finalizar() { free(vetorPrincipal->vetorAuxiliar); }